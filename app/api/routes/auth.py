...
async def auth_status(
    ...
    auth_service: AccountAuthService = Depends(get_auth_service),
    notebook_service: NotebookLMService = Depends(get_notebook_service),
) -> AuthStatusResponse:
    return await auth_service.get_status(account.id, notebook_service)
...
async def login_start(
    ...
    return auth_service.start_login_flow(account.id)
...
async def login_complete(
    ...
    response = auth_service.complete_login_flow(
